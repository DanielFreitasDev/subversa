...
- `prettier-plugin-java` is pinned **exactly** to 2.6.8 (chevrotain, pure JS): 2.7+ switched to web-tree-sitter **WASM**, which breaks under Vite dev and inside the Tauri WebView. Don't bump it without solving wasm serving end-to-end.
...
### Frontend (`src`)
- `lib/api.ts` — typed `invoke` wrapper per command. `lib/types.ts` — TS mirror of the Rust types. `lib/op.ts` — uniform result handling (`reportOutput`, `tryRun`, `extractRevision`). `lib/diff.ts` — unified-diff parser. `lib/findreplace.ts`/`lib/fuzzy.ts`/`lib/indent.ts` — pure engines behind the embedded editor (find/replace, go-to-file, indent detection). `lib/utils.ts` — `cn()` and the status→visual map.
